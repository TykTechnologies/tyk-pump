...
- 6. [Running The Full Test Suite](#running-the-full-test-suite)
...
This command runs only the tests that need no external services. For the checks that CI performs, see
[Running The Full Test Suite](#running-the-full-test-suite).

## Running The Full Test Suite

The `go test -v ./...` command in the previous section compiles and runs the tests, but it does not reproduce what CI
checks. Tests that need a database skip themselves when the matching environment variable is absent, and tests that need
Redis fail when no server is reachable. A run without the supporting services can therefore look successful while
skipping 53 SQL tests, or fail for reasons unrelated to your change.

CI runs `bin/ci-test.sh`. That script:

1. Runs `go vet` on the root package.
2. Checks formatting with `gofmt -s` and import order with `goimports`.
3. Runs `go test` for every package under `github.com/TykTechnologies/tyk-pump/...`.
4. Runs the `pumps` package twice, once with `MONGO_DRIVER=mgo` and once with `MONGO_DRIVER=mongo-go`.
5. Repeats every package with `GOEXPERIMENT=boringcrypto`.

The script stops at the first failing package, so a failure early in the run hides every package after it.

### Required Services

The suite expects four services reachable from the machine that runs the tests. These versions match the ones CI uses:
...
PostgreSQL and MySQL both need a database named `tyk_analytics` owned by user `tyk` with password `tyk123`, or
equivalent credentials that you supply through the environment variables in the next section.

Any equivalent instance works: a container runtime, a local package installation, or a remote host that you point the
environment variables at. If you use Docker Compose, the `docker-compose-ci-tests.yml` file in the repository root
starts all four services at the versions above:

```
docker compose -f docker-compose-ci-tests.yml up -d --wait
```

The `--wait` flag blocks until every health check passes. Without it, the tests can start before MySQL accepts
connections.

If another process already holds one of the ports, that container fails to start while the other three report healthy,
and the suite then runs against a missing service. To check the ports before you start, run:

```
lsof -i :6379 -i :27017 -i :5432 -i :3306
```

### Environment Variables

Export these variables before you run the suite. Adjust the hosts, ports, and credentials if your services do not match
the defaults in the previous section:

```
export TYK_PMP_ANALYTICSSTORAGETYPE=redis
export TYK_PMP_ANALYTICSSTORAGECONFIG_TYPE=redis
export TYK_PMP_ANALYTICSSTORAGECONFIG_HOST=localhost
export TYK_PMP_ANALYTICSSTORAGECONFIG_PORT=6379
export TYK_TEST_POSTGRES="host=localhost user=tyk password=tyk123 dbname=tyk_analytics port=5432 sslmode=disable"
export TYK_TEST_MYSQL="tyk:tyk123@tcp(localhost:3306)/tyk_analytics?parseTime=true"
```

`TYK_TEST_POSTGRES` and `TYK_TEST_MYSQL` gate the SQL tests. When either variable is unset, every test for that database
reports `SKIP` rather than failing, which is easy to miss in a long log.

### Run The Suite

Go caches successful test results, and the cache key does not include the state of external services. A cached result
survives a service restart, a version change, or a service that stopped running, so a second run can report `ok` without
contacting anything. Clear the test cache first:
...
To skip the cache for a single run instead of clearing it, set `GOFLAGS=-count=1`:

```
GOFLAGS=-count=1 ./bin/ci-test.sh
```

To capture the log the way CI does, redirect both streams. The script writes the commands it runs to stderr, so a
stdout-only capture loses them along with any failure message:
...
A complete run exits with status 0 and reports `ok` for `tyk-pump`, `analytics`, `analytics/demo`, `logger`, `pumps`,
`serializer`, and `storage`. The `analytics/proto`, `retry`, and `server` packages contain no tests and report `0.0% of
statements`.

Confirm all three of the following:

- No line contains `(cached)`. A cached line means those tests did not run.
- No line reports `Skipping test because TYK_TEST_POSTGRES environment variable is not set`, or the same message for
  `TYK_TEST_MYSQL`. Either message means the SQL tests did not run.
- The only skipped tests are `TestMongoPump_capCollection_Not64arch`, which skips on 64-bit architectures, and
  `TestSegmentPump`, which needs a Segment write key.

### Lint

CI also runs `golangci-lint` against the configuration in `.golangci.yaml`, which uses the version 2 format. CI pins
golangci-lint to v2.12.2 and passes `--issues-exit-code=0`, so lint findings appear in the job output and in the
SonarQube report without failing the build. The `bin/ci-test.sh` script does not run the linter, so run it separately:
...
The tests leave tables behind in the PostgreSQL database, so consecutive runs share accumulated schema. To start from an
empty database, recreate the services rather than restarting them. With Docker Compose, the `-v` flag removes the
volumes:

```
docker compose -f docker-compose-ci-tests.yml down -v
```

The script also writes a `<package>.cov` coverage profile per package into the working directory. The `.gitignore` file
already excludes these files.
