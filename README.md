...
- 5. [Tyk Analytics Schema](#tyk-analytics-schema)
...
| `APIKey` | `Request authentication key` | If hashing is enabled then the value will be hashed, otherwise it will be the original value. If no API key is provided then gateway will substitute a default value. | Either `auth_key` (unhashed) or `6129dc1e8b64c6b4` (hashed) for `auth_key`, depending on hashing configuration. Otherwise `00000000`, if no authentication provided. |
