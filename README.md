...
When it does this, the Pump will halve the `current_aggregation_time ` so that it aggregates records for half the time period originally configured before creating a new document, reducing the risk of repeatedly hitting the maximum document size.

This self healing is repeatable, however, such that if the document size does reach maximum (16MB) even with the new shorter aggregation period, a new document will be created and the `current_aggregation_time` will be halved again.

The minimum value for `current_aggregation_time` is 1; Self Healing cannot reduce it beyond this value.

For example, if the `current_aggregation_time` is configured as 50 (minutes) but the document hits the maximum size (16MB), a new document will be started and the `current_aggregation_time` will be set to 25 (minutes).

Note that `store_analytics_per_minute` takes precedence over `current_aggregation_time` so if `store_analytics_per_minute` is equal to true, the value of `current_aggregation_time` will be equal to 1 and self healing will not operate.
