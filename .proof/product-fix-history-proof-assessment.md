...
| 6820131 | 2023-06-12 | [TT-9126] Fix error log when omit_configfile option is enabled | `covered_by_existing_proof`: SW-REQ-002 already models the omit-config-file gate as `config_file_enabled -> json_loaded_before_env_override`; `TestIgnoreConfig` covers omit=true with an existing config file, omit=false, env unset, and missing-file fallback. `proof mcdc show SW-REQ-002` reports all rows covered and 100% code-level MC/DC across the matched config functions, so no new requirement, KI, or signal was needed for this historical fix. |
