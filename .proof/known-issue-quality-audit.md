...
| `mcp-sql-aggregate-mysql-create-index-syntax-broken` | KnownIssue | `needs_reproducer_hardening` | Static SQL construction shows `CREATE INDEX IF NOT EXISTS`; a MySQL dialect witness would better prove the actual backend rejection. | Attached and refreshed `.proof/evidence/ki-mcp-sql-aggregate-mysql-create-index-syntax-broken.yaml`; backlog remains to add MySQL syntax/rejection tripwire. |
...
| Item | Type | Verdict | Evidence Checked | Action |
| --- | --- | --- | --- | --- |
| `no-panic-recovery-in-exec-pump-writing` | KnownIssue | `needs_merge` -> evidenced | Duplicate of the `main.go` fanout panic surface; stronger subprocess evidence is on `pump-fanout-panic-not-recovered`, while legacy specs/problem reports still reference this ID. | Attached and refreshed `.proof/evidence/ki-no-panic-recovery-in-exec-pump-writing.yaml` using the same canonical subprocess tripwire; backlog remains to migrate old references and withdraw/merge this alias. |
| `preprocess-decode-error-leaves-nil-hole-in-keys` | KnownIssue | `needs_reproducer_hardening` | Static nil-hole evidence is current, but the existing focused test only uses `Pumps=nil`, so it proves decode logging, not the nil slot reaching `filterData`. | Attached and refreshed `.proof/evidence/ki-preprocess-decode-error-leaves-nil-hole-in-keys.yaml`; backlog remains to add a bad serialized record plus filtering/decoding pump test that reaches the nil-hole panic. |
...
| `sql-aggregate-sharded-shared-db-race` | KnownIssue | `needs_reproducer_hardening` | Shared `c.db` mutation is real, but no race/concurrency witness exists. | Attached and refreshed `.proof/evidence/ki-sql-aggregate-sharded-shared-db-race.yaml`; backlog remains to add focused `go test -race` sharded concurrent `WriteData` test. |
...
| `sql-aggregate-upsert-order-undocumented` | KnownIssue | `needs_metadata_hardening` | Valid documentation/proof gap, but product disposition still needs tightening. | Attached and refreshed `.proof/evidence/ki-sql-aggregate-upsert-order-undocumented.yaml`; backlog remains to document unordered/id-keyed semantics in `SW-REQ-067`. |
