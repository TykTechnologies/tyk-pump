...
| `sql-aggregate-atomicity-fault-injection-missing` | KnownIssue | `needs_reproducer_hardening` | Correct proof gap, but current reproducer is still static evidence rather than a failure-injection witness. | Attached and refreshed `.proof/evidence/ki-sql-aggregate-atomicity-fault-injection-missing.yaml`; backlog remains to add fake-driver or DB-backed failure injection for create/transaction/deadlock paths. |
| `sql-aggregate-background-index-concurrency-unbounded` | KnownIssue | `needs_reproducer_hardening` | Lifecycle gap exists, but there is no bounded failure/cancellation witness. | Attached and refreshed `.proof/evidence/ki-sql-aggregate-background-index-concurrency-unbounded.yaml`; backlog remains to add non-container dry-run/fake-DB or race witness. |
