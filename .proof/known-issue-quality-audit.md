...
| `serializer-protobuf-loses-city-names` | KnownIssue | `needs_reproducer_hardening` | Real bug exists in protobuf geo city handling, but current evidence is static only. | Attached and refreshed `.proof/evidence/ki-serializer-protobuf-loses-city-names.yaml`; backlog remains to add `TestProtobuf_GeoCityNamesLoss_KI` with `// Reproduces`. |
