...
## Intent
Realises GraphQL-aware record shaping for parent **SYS-REQ-002**. `(*AnalyticsRecord).ToGraphRecord` projects an analytics record into a `GraphRecord` suitable for persistence in a SQL or Mongo store, copying the embedded `AnalyticsRecord` plus the GraphQL-specific fields from `GraphQLStats`: `OperationType` (mapped from the enum to a human-readable `"Query"`/`"Mutation"`/`"Subscription"`), `RootFields`, `Types`, `Errors`, `HasErrors`, `Variables`. It does not reparse legacy raw request/response/schema payloads. If the record's `IsGraphRecord()` returns false (i.e. `GraphQLStats.IsGraphQL == false`), it returns a zero-value `GraphRecord` so the caller can decide what to do with non-GraphQL traffic.

## Motivation
GraphQL pumps need columns/document fields that don't exist on the base analytics record, but the source data already lives on `AnalyticsRecord.GraphQLStats`. A dedicated projection type keeps the persistence layer's schema explicit (`Types map[string][]string`, `RootFields []string`, `Errors []GraphError`) without polluting `AnalyticsRecord` itself. Making `GraphQLStats` authoritative also avoids the historical unresolved-subgraph parser failure from TT-6012: malformed or federation-specific legacy schema payloads are preserved as record data, not reparsed by pump. The extra `if a.ResponseCode >= 400 { record.HasErrors = true }` rule means HTTP-level failures are surfaced as GraphQL errors even when the resolver produced no `errors` array — useful for dashboards that key off `has_errors` regardless of whether the failure was transport or resolver.
...
- `analytics/graph_record_test.go:TestAnalyticsRecord_ToGraphRecord_IgnoresUnresolvedLegacySubgraphSchema` — proves a historical unresolved-subgraph query/schema payload does not override or break structured `GraphQLStats` projection.
