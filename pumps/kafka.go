...
func (k *KafkaPump) WriteData(ctx context.Context, data []interface{}) error {
	...
	for i, v := range data {
		...
		if jsonError != nil { //mcdc:ignore:defensive json.Marshal of map[string]interface{} with basic Go types (string/int/time.Time/[]string/map[string]string) cannot fail — KI kafka-writedata-marshal-err-structurally-unreachable
