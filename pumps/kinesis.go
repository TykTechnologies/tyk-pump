...
func (p *KinesisPump) Init(config interface{}) error {
	...
	if p.kinesisConf.BatchSize == 0 { //mcdc:ignore:capability-gap production Init runs after LoadDefaultConfig which requires real AWS credentials/environment; all kinesis-pump tests use TestableKinesisPump.InitWithMock that mirrors this branch (covered by TestKinesisPump_BatchSize_Configuration). Driving the production Init.BatchSize==0 arm requires a live AWS config which is not available in unit tests. KI mcdc-pumps-below-95. [ki: mcdc-pumps-below-95]
		p.kinesisConf.BatchSize = defaultBatchSize
	}

	if p.kinesisConf.StreamName == "" { //mcdc:ignore:external-evidence production Init runs after LoadDefaultConfig (requires AWS env); the mirror branch in TestableKinesisPump.InitWithMock is exercised by TestKinesisPump_StreamName_Required. KI mcdc-pumps-below-95.
	...
	if p.kinesisConf.KMSKeyID != "" { //mcdc:ignore:external-evidence production Init requires AWS env to reach this branch; the mirror branch in TestableKinesisPump.InitWithMock is exercised by TestKinesisPump_DescribeStream_* tests. KI mcdc-pumps-below-95.
		...
		default:
			...
			if err != nil { //mcdc:ignore:external-evidence production StartStreamEncryption err arm requires a live AWS failure; mirror branch in TestableKinesisPump.InitWithMock is exercised by TestKinesisPump_DescribeStream_NotEncrypted_StartEncryptionResourceInUse / _StartEncryptionGenericError. KI mcdc-pumps-below-95.
				var resourceInUseErr *types.ResourceInUseException
				if errors.As(err, &resourceInUseErr) { //mcdc:ignore:capability-gap both arms covered by the testable-mock branch above (ResourceInUse / Generic). KI mcdc-pumps-below-95. [ki: mcdc-pumps-below-95]
...
func (p *KinesisPump) WriteData(ctx context.Context, records []interface{}) error {
	...
	for _, batch := range batches {
		...
		for _, record := range batch {
			...
			decoded, ok := record.(analytics.AnalyticsRecord) //mcdc:ignore:defensive !ok arm is unreachable from the production call path: WriteData is only invoked by pump.Pump with []interface{} containing analytics.AnalyticsRecord values. KI mcdc-pumps-below-95.
			if !ok {                                          //mcdc:ignore:defensive same rationale as the type-assert above — the !ok arm is structurally unreachable from production. KI mcdc-pumps-below-95.
			...
			if jsonError != nil { //mcdc:ignore:defensive json.Marshal on a flat Json map of primitive fields (string/int64/time.Time/[]string) cannot fail. KI mcdc-pumps-below-95.
				p.log.WithError(jsonError).Error("unable to marshal message")
			}

			n, err := rand.Int(rand.Reader, big.NewInt(1000000000))
			if err != nil { //mcdc:ignore:capability-gap crypto/rand.Int only fails when /dev/urandom or BCryptGenRandom is unavailable — unreachable on supported platforms. KI mcdc-pumps-below-95. [ki: mcdc-pumps-below-95]
		...
		if err != nil { //mcdc:ignore:capability-gap err arm requires a live AWS Kinesis endpoint to fail — production WriteData uses the real *kinesis.Client (not the SDK interface), so unit tests cannot inject failures without rewriting the function signature. KI mcdc-pumps-below-95. [ki: mcdc-pumps-below-95]
			p.log.Error("failed to put records to Kinesis: ", err)
		}

		// Check for failed records
		if output != nil { //mcdc:ignore:capability-gap output==nil branch only fires after a PutRecords error which itself is unreachable from unit tests (above mcdc:ignore). KI mcdc-pumps-below-95. [ki: mcdc-pumps-below-95]
			for _, record := range output.Records {
				if record.ErrorCode != nil { //mcdc:ignore:capability-gap record.ErrorCode is populated only on a per-record AWS API failure; production WriteData cannot reach a real AWS endpoint from a unit test. KI mcdc-pumps-below-95. [ki: mcdc-pumps-below-95]
