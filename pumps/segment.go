...
func (s *SegmentPump) WriteDataRecord(record analytics.AnalyticsRecord) error {
	...
	if err != nil { //mcdc:ignore:defensive ToJSONMap delegates to json.Marshal of an analytics.AnalyticsRecord — all fields are JSON-encodable so the err arm is structurally unreachable. KI mcdc-pumps-below-95.
		...
		if err != nil { //mcdc:ignore:defensive segment.Client.Track only returns an error when the SDK's input validation rejects the message; AnonymousId and Event are always set here so the validation path is unreachable in production. KI mcdc-pumps-below-95.
...
func (s *SegmentPump) ToJSONMap(obj interface{}) (map[string]interface{}, error) {
	...
	if err != nil { //mcdc:ignore:defensive json.Unmarshal of bytes just produced by json.Marshal of an AnalyticsRecord into map[string]interface{} cannot fail. KI mcdc-pumps-below-95.
		return nil, err
	}
