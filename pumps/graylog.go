...
// reqproof:implements SW-REQ-049
...
func (p *GraylogPump) WriteData(ctx context.Context, data []interface{}) error {
	...
	if p.client == nil { //mcdc:ignore:defensive p.client is always set by GraylogPump.Init -> connect(); the nil-check is defensive plumbing in case the pump is constructed without Init (no public surface drives it)
		p.connect()
	...
	for _, item := range data {
		...
		if err != nil { //mcdc:ignore:defensive json.Marshal cannot fail on the map[string]interface{} composed exclusively of mapping[key] values (string/int/int64) — KI graylog-moesif-logfatal-on-record-error
			p.log.Fatal(err)
		}
		...
		if err != nil { //mcdc:ignore:defensive json.Marshal cannot fail on gelfData (string host, int64 timestamp, string message) — KI graylog-moesif-logfatal-on-record-error
			p.log.Fatal(err)
		}
