...
// reqproof:implements SW-REQ-041
...
func (c *SQLAggregatePump) Init(conf interface{}) error {
	...
	if err := HandleTableMigration(c.db, &c.SQLConf.SQLConf, analytics.AggregateSQLTable, &analytics.SQLAnalyticsRecordAggregate{}, c.log, migrateShardedTables); err != nil { //mcdc:ignore:capability-gap HandleTableMigration err arm needs fake migrator; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
		return err
	}
	...
	if !c.SQLConf.TableSharding {
		...
		if err := c.ensureTable(analytics.AggregateSQLTable); err != nil { //mcdc:ignore:capability-gap ensureTable err arm needs fake migrator; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
			return err
		}

		// we can run the index creation in background only for postgres since it supports CONCURRENTLY
		shouldRunOnBackground := false
		if c.dbType == "postgres" { //mcdc:ignore:capability-gap non-postgres Init arm requires Dialect(sqlite) which is rejected; covered structurally via TestMCDC_SQLAggregatePump_Sharded direct wiring; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
			shouldRunOnBackground = true
			c.backgroundIndexCreated = make(chan bool, 1)
		}

		// if index doesn't exist, create it
		if err := c.ensureIndex(analytics.AggregateSQLTable, shouldRunOnBackground); err != nil { //mcdc:ignore:capability-gap ensureIndex err arm needs fake DB seam; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
			c.log.Error(err)
			return err
		}
	}
...
// reqproof:implements SW-REQ-066
...
// reqproof:implements SW-REQ-065
func (c *SQLAggregatePump) ensureTable(tableName string) error {
	if !c.db.Migrator().HasTable(tableName) {
		c.db = c.db.Table(tableName)

		if err := c.db.Migrator().CreateTable(&analytics.SQLAnalyticsRecordAggregate{}); err != nil { //mcdc:ignore:capability-gap CreateTable err arm needs fake migrator; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
...
// reqproof:implements SW-REQ-064
func (c *SQLAggregatePump) WriteData(ctx context.Context, data []interface{}) error {
	...
	for i := 0; i <= dataLen; i++ {
		if c.SQLConf.TableSharding {
			...
			if err := c.ensureIndex(table, false); err != nil { //mcdc:ignore:capability-gap ensureIndex err arm needs fake DB seam; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
				return err
			}
		} else {
...
// reqproof:implements SW-REQ-067
