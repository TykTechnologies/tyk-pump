...
func (g *MCPSQLPump) Init(conf interface{}) error {
	...
	if name := g.Conf.TableName; name != "" { //mcdc:ignore:capability-gap both arms exist in suite but proof mcdc measure samples only one; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
		g.tableName = name
	}
	...
	if err := HandleTableMigration(g.db, &g.Conf.SQLConf, g.tableName, &analytics.MCPRecord{}, g.log, migrateShardedTables); err != nil { //mcdc:ignore:capability-gap HandleTableMigration err arm needs fake migrator; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
		return err
	}
...
func (g *MCPSQLPump) writeMCPBatch(ctx context.Context, recs []*analytics.MCPRecord) {
	for ri := 0; ri < len(recs); ri += g.Conf.BatchSize {
		...
		if tx := g.db.WithContext(ctx).Create(recs[ri:ends]); tx.Error != nil { //mcdc:ignore:capability-gap tx.Error arm needs fake DB seam; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
...
func (g *MCPSQLPump) ensureMCPShardedTable(recDate string) {
	...
	if !g.db.Migrator().HasTable(table) {
		if err := g.db.AutoMigrate(&analytics.MCPRecord{}); err != nil { //mcdc:ignore:capability-gap AutoMigrate err arm needs fake migrator; KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
