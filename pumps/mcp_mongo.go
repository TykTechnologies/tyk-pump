...
func (g *MCPMongoPump) insertMCPDataSet(dataSet []model.DBObject, collectionName string, errCh chan error) {
	...
	if err != nil {
		...
		if strings.Contains(strings.ToLower(err.Error()), "closed explicitly") { //mcdc:ignore:capability-gap "closed explicitly" sentinel is emitted only by the mgo driver when a session is closed via SessionWithoutCopy + explicit Close; the official mongo-go driver used by tests surfaces network errors as "connection refused" / "context deadline exceeded" instead, so this T-arm is unreachable from external tests — KI mcdc-pumps-below-95 [ki: mcdc-pumps-below-95]
