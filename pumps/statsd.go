...
func (s *StatsdPump) WriteData(ctx context.Context, data []interface{}) error {
	...
	for _, v := range data {
		...
		for i, t := range s.dbConf.Tags {
			...
			if err != nil { //mcdc:ignore:defensive json.Marshal cannot fail on the canonical mapping values (string/int/int64/time.Time) emitted by getMappings — KI pumps-logfatal-on-config-decode (no behavioural impact)
				tag = ""
			} else {
				tag = string(b)
				// Lowercase
		...
		for _, f := range s.dbConf.Fields {
			if s.isTimingField(f) {
				if v, ok := mapping[f]; ok { //mcdc:ignore:defensive ok=F unreachable: isTimingField returns true only for keys ("request_time","latency_total","latency_upstream","latency_gateway") that are unconditionally inserted into getMappings — ok is always T
					if iv, ok2 := v.(int64); ok2 { //mcdc:ignore:defensive ok2=F unreachable: getMappings stores all four timing-field values as int64 (RequestTime is int64; Latency.* are int64), so the type-assertion always succeeds
